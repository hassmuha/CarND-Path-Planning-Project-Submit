        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            for (int i=0; i<= 50-previous_path_x.size();i++){
              //TODO move this outside of loop as it does not depends upon the loop
              double N = target_dist/(0.02*ref_vel/2.24); //N(no of equally distance points) * .02(simulator update time) * ref_vel(miles/s) = target_dist
